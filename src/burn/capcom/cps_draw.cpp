...
#ifndef BUILD_PSP
		for (int i = 384 * 224 / 16; i > 0 ; i--) {
#else
		for (int i = (nBurnPitch / nBurnBpp) * 224 / 16; i > 0 ; i--) {
#endif
			*pClear++ = nColour;
			*pClear++ = nColour;
			*pClear++ = nColour;
...
#ifndef BUILD_PSP
		memset(pBurnDraw, 0, 384 * 224 * sizeof(short));
#else		
		memset(pBurnDraw, 0, (nBurnPitch / nBurnBpp) * 224 * sizeof(short));
#endif
	}
}
