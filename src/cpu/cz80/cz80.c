...
u32 Cz80_Reset()
{
	...
	memset(CPU, 0, (u32)(&(CPU->IntVect)) - (u32)(&(CPU->BC)));
...
{
	if (cpu->Read[adr >> CZ80_FETCH_BITS])
		return (cpu->Read[adr >> CZ80_FETCH_BITS])[adr];
	else
...
u32 Cz80_Read_Word(cz80_struc *cpu, u32 adr)
{
	...
	if (cpu->Read[adr >> CZ80_FETCH_BITS])
		return *((u16*)(cpu->Read[adr >> CZ80_FETCH_BITS] + adr));
	else
...
#else
	if (cpu->Read[adr >> CZ80_FETCH_BITS])
		return ((u32)((cpu->Read[adr >> CZ80_FETCH_BITS])[adr]) << 8) | (u32)((cpu->Read[adr >> CZ80_FETCH_BITS])[adr + 1]);
	else
...
{
	if (cpu->Write[adr >> CZ80_FETCH_BITS])
		(cpu->Write[adr >> CZ80_FETCH_BITS])[adr] = data;
	else
...
#if CZ80_USE_WORD_HANDLER
	if (cpu->Read[adr >> CZ80_FETCH_BITS])
		*((u16*)(cpu->Read[adr >> CZ80_FETCH_BITS] + adr)) = data;
	else
...
#else
	if (cpu->Read[adr >> CZ80_FETCH_BITS]) {
		(cpu->Write[adr >> CZ80_FETCH_BITS])[adr] = data >> 8;
		(cpu->Write[adr >> CZ80_FETCH_BITS])[adr + 1] = data & 0xFF;
	} else {
...
void FASTCALL Cz80_Set_PC(cz80_struc *cpu, u32 val)
{
    ...
    cpu->BasePCData = (u32) cpu->FetchData[val >> CZ80_FETCH_SFT];
    cpu->PC = val + cpu->BasePC;
    cpu->PCData = val + cpu->BasePCData;
}
